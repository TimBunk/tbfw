...
std::vector<Mesh*> OBJloader::LoadObject(const char * fileName, bool enableTextCoord, bool hasTextCoord, bool enableNormCoord, bool hasNormCoord)
{
	...
	if (!obj.is_open()) {
		...
		while (moreMeshes) {
			...
			if (hasTextCoord && hasNormCoord) {
				...
				vertices = OBJloader::LoadFaces(obj, reader, moreMeshes, verticesArraySize, vertexIndices, textureIndices, normalIndices, tmp_vertices, tmp_textures, tmp_normals);
			}
			...
			Mesh* mesh;
			mesh = new Mesh(meshData, enableTextCoord, hasTextCoord, enableNormCoord, hasNormCoord);
